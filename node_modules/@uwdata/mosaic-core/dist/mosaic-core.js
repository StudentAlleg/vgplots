...
function collapseContiguousByteRanges(chunks) {
  ...
  for (let x2, y2, i2 = 0, j2 = 0, n = chunks.length; ++i2 < n; ) {
    x2 = result[j2];
    y2 = chunks[i2];
...
function valueToString(x2) {
  ...
  if (ArrayBuffer.isView(x2)) {
    if (x2 instanceof BigInt64ArrayCtor || x2 instanceof BigUint64ArrayCtor) {
...
var Visitor = class {
  visitMany(nodes, ...args) {
    return nodes.map((node, i2) => this.visit(node, ...args.map((x2) => x2[i2])));
  }
  visit(...args) {
...
var MakeDataVisitor = class extends Visitor {
  ...
  visitNull(props) {
    ...
    return new Data(type, offset, length2, 0);
  }
  visitBool(props) {
  ...
  visitUnion(props) {
    ...
    const { ["length"]: length2 = typeIds.length, ["nullCount"]: nullCount = props["nullBitmap"] ? -1 : 0 } = props;
    if (DataType.isSparseUnion(type)) {
      return new Data(type, offset, length2, nullCount, [void 0, void 0, nullBitmap, typeIds], children);
    }
    const valueOffsets = toInt32Array(props["valueOffsets"]);
    return new Data(type, offset, length2, nullCount, [valueOffsets, void 0, nullBitmap, typeIds], children);
  }
...
var GetByteLengthVisitor = class extends Visitor {
  ...
  visitDictionary(data, i2) {
    var _a5;
    return data.type.indices.bitWidth / 8 + (((_a5 = data.dictionary) === null || _a5 === void 0 ? void 0 : _a5.getByteLength(data.values[i2])) || 0);
  }
};
...
var ByteBuffer = class {
  ...
  __string(offset, opt_encoding) {
    ...
    while (i2 < length2) {
      let codePoint;
      const a = this.readUint8(offset + i2++);
      if (a < 192) {
        codePoint = a;
      } else {
        const b2 = this.readUint8(offset + i2++);
        if (a < 224) {
          codePoint = (a & 31) << 6 | b2 & 63;
        } else {
          const c = this.readUint8(offset + i2++);
          if (a < 240) {
            codePoint = (a & 15) << 12 | (b2 & 63) << 6 | c & 63;
          } else {
            const d = this.readUint8(offset + i2++);
...
var VectorLoader = class extends Visitor {
  ...
  visitDenseUnion(type, { length: length2, nullCount } = this.nextFieldNode()) {
    return makeData({ type, length: length2, nullCount, nullBitmap: this.readNullBitmap(type, nullCount), typeIds: this.readTypeIds(type), valueOffsets: this.readOffsets(type), children: this.visitMany(type.children) });
  }
  visitSparseUnion(type, { length: length2, nullCount } = this.nextFieldNode()) {
    return makeData({ type, length: length2, nullCount, nullBitmap: this.readNullBitmap(type, nullCount), typeIds: this.readTypeIds(type), children: this.visitMany(type.children) });
  }
...
function compareUnion(type, other) {
  return type === other || compareConstructor(type, other) && type.mode === other.mode && type.typeIds.every((x2, i2) => x2 === other.typeIds[i2]) && instance7.compareManyFields(type.children, other.children);
}
  ...
  for (let i2 = -1, n = columns.length; ++i2 < n; ) {
    ...
    if (length2 >= batchLength) {
      if (length2 === batchLength) {
        ...
        memo.numBatches = Math.max(memo.numBatches, columns[i2].unshift(child.slice(batchLength, length2 - batchLength)));
      }
    } else {
      const field = fields[i2];
    ...
    const unwrap = (x2) => {
      if (x2) {
        if (x2 instanceof RecordBatch) {
          ...
          const schema2 = new Schema2(keys.map((k2, i2) => new Field2(String(k2), vecs[i2].type)));
...
var RecordBatch = class {
  constructor(...args) {
    switch (args.length) {
      ...
      case 1: {
        const [obj] = args;
        const { fields, children, length: length2 } = Object.keys(obj).reduce((memo, name, i2) => {
...
function encodeBufferRegion(b2, node) {
  return Buffer2.createBuffer(b2, new Long4(node.offset, 0), new Long4(node.length, 0));
}
...
var VectorAssembler = class extends Visitor {
  ...
  visit(data) {
    ...
    const { type } = data;
    if (!DataType.isDictionary(type)) {
      const { length: length2, nullCount } = data;
...
function assembleUnion(data) {
  ...
  if (type.mode === UnionMode.Sparse) {
    ...
    if (data.offset <= 0) {
      ...
      const unshiftedOffsets = rebaseValueOffsets(-valueOffsets[0], length2, valueOffsets);
...
function assembleFlatListVector(data) {
  ...
  const lastOffset = valueOffsets[length2];
  const byteLength = Math.min(lastOffset - firstOffset, values.byteLength - firstOffset);
  addBuffer.call(this, rebaseValueOffsets(-valueOffsets[0], length2, valueOffsets));
  ...
  switch (typeof value) {
    ...
    default:
      if (value == null) {
        ...
        return ts === Date.UTC(y2, m2, d) ? `MAKE_DATE(${y2}, ${m2 + 1}, ${d})` : `EPOCH_MS(${ts})`;
...
function logical(op, clauses) {
  ...
  const strings = children.map((c, i2) => i2 ? ` ${op} ` : "");
  if (clauses.length)
    strings.push("");
...
var binaryOp = (op) => (a, b2) => sql`(${asColumn(a)} ${op} ${asColumn(b2)})`.annotate({ op, a, b: b2, visit });
var eq = binaryOp("=");
var neq = binaryOp("<>");
var lt = binaryOp("<");
var gt = binaryOp(">");
...
function fnv_hash(v2) {
  let a = 2166136261;
  for (let i2 = 0, n = v2.length; i2 < n; ++i2) {
    const c = v2.charCodeAt(i2);
    const d = c & 65280;
    if (d)
...
function getActiveView(clause) {
  ...
  if (type === "interval" && scales) {
    ...
    if (bins.length === 1) {
      ...
      predicate = (p2) => p2 ? and(p2.children.map(({ range }, i2) => isBetween(`active${i2}`, range.map(bins[i2])))) : [];
      columns = Object.fromEntries(
        clause.predicate.children.map((p2, i2) => [`active${i2}`, bins[i2](p2.field)])
      );
    }
  } else if (type === "point") {
    predicate = identity;
...
function getBaseTable(query) {
  ...
  for (let i2 = 1; i2 < subq.length; ++i2) {
    const from = getBaseTable(subq[i2]);
    if (from === void 0)
      continue;
    if (from !== base)
      return NaN;
...
var E = class {
  ...
  async send(e) {
    ...
    let t = new p(this._bindings, this._conn, r), n = await RecordBatchReader.from(t);
...
var b = class {
  ...
  async send(...e) {
    let r = await this.bindings.sendPrepared(this.connectionId, this.statementId, e), t = new p(this.bindings, this.connectionId, r), n = await RecordBatchReader.from(t);
    return console.assert(n.isAsync()), console.assert(n.isStream()), n;
  }
};
